...
bool GameScene::init()
{
	...
	for (int i = 0; i < 3; i++)
	{
		...
		while (monstro_num < MONSTRONUM[map_mossion][i])
		{
			...
			int x = random(5, MAPSIZE-2);
			int y = random(5, MAPSIZE-2);
...
void GameScene::onTouchEnded(Touch *touch, Event *unused_event)
{
	...
	if (!(fabs(dy - y) < 100 && fabs(dx - x) < 100))	//判断方向
	{
		if (!isMove)
		{
			...
			if (maze[hi][hj] == 1)
			{
				if ((hi == 16 && dirc_now == 0) || (hj == 5 && dirc_now == 1) || (hi == 5 && dirc_now == 2) || (hj == 17 && dirc_now == 3)) //上下块左右动 左右块上下动 
					bgMoveTo(dirc_now);
				else
				{
					if ((hi < 6 || hi>15) && (hj < 6 || hj>16)) //四角块
						hero->Hero::moveTo(dirc_now);
					else
					{
						if ((hi >= 6 && hi <= 15) && (hj < 6 || hj>16) && (dirc_now == 1 || dirc_now == 3)) //左右块左右动
							hero->Hero::moveTo(dirc_now);
						else
						{
							if ((hi < 6 || hi>15) && (hj >= 6 && hj <= 16) && (dirc_now == 0 || dirc_now == 2)) //上下块上下动
								hero->Hero::moveTo(dirc_now);
							else //中心块
								bgMoveTo(dirc_now);
						}
...
void GameScene::heroFight()
{
	...
	this->runAction(Sequence::create(CallFunc::create([&](){fight(true); }), DelayTime::create(1.0f), CallFunc::create([&](){
		...
		int hero_hp = UserDefault::sharedUserDefault()->getIntegerForKey("monstro_hp");
		if (monstro_hp > 0)
		{
